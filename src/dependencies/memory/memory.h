...
namespace memory
{
	...
	T* capture_interface(const char* module_name, const char* interface_name)
	{
		...
		const auto temp = reinterpret_cast<interface_type>(GetProcAddress(GetModuleHandleA(module_name), xorstr("CreateInterface")));
		printf(xorstr("Captured interface '%s' in '%s'\n"), interface_name, module_name);
	...
	T* get_vmt_from_instruction(uintptr_t address)
	{
		...
		printf(xorstr("Got VMT from address '%p'\n"), real_address);
	...
	T* get_vmt_from_instruction(uintptr_t address, size_t offset)
	{
		...
		printf(xorstr("Got VMT from address '%p' at offset '%p'\n"), address, offset);
